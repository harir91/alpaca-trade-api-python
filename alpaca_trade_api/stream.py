...
class DataStream:
    ...
    async def _connect(self):
        self._ws = await websockets.connect(
            self._endpoint, extra_headers={"Content-Type": "application/msgpack"}
        )
    ...
    async def _dispatch(self, msg):
        ...
        if msg_type == "t":
            ...
            handler = self._bar_handlers.get(symbol, self._bar_handlers.get("*", None))
            if handler:
                await handler(self._cast(msg_type, msg))
        elif msg_type == "d":
            ...
            log.info(
                f'subscribed to trades: {msg.get("trades", [])}, '
    ...
    async def _subscribe_all(self):
        if (
            ...
            or self._status_handlers
            or self._luld_handlers
        ):
            await self._ws.send(
                msgpack.packb(
                    {
    ...
    async def _unsubscribe(
        self, trades=(), quotes=(), bars=(), daily_bars=(), statuses=(), lulds=()
    ):
    ...
    def unsubscribe_bars(self, *symbols):
        if self._running:
            asyncio.get_event_loop().run_until_complete(self._unsubscribe(bars=symbols))
        for symbol in symbols:
        ...
        while True:
            try:
                if self._conn_error or not self._running:
                    if retries > 0:
                        log.warn("attempting to reconnect... attempt " + str(retries))
                ...
                if retries > 1:
                    await asyncio.sleep(int(os.environ.get("APCA_RETRY_WAIT", 3)))
                log.warn("websocket error: " + str(wse))
...
class TradingStream:
    ...
    def subscribe_trade_updates(self, handler):
        ...
        if self._running:
            asyncio.get_event_loop().run_until_complete(self._subscribe_trade_updates())
...
class Stream:
    def __init__(
        ...
        self._trading_ws = TradingStream(self._key_id, self._secret_key, self._base_url)
        self._data_ws = DataStream(
            self._key_id, self._secret_key, self._data_steam_url, raw_data, data_feed
        )
    ...
    async def _run_forever(self):
        await asyncio.gather(
            self._trading_ws._run_forever(), self._data_ws._run_forever()
        )
