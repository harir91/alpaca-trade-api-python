...
import os
import websockets
from .entity import Quote, Trade, Agg, Entity, trade_mapping, quote_mapping, agg_mapping
...
class StreamConn(object):
    ...
    async def authenticate(self):
        ...
        await ws.send(json.dumps({"action": "auth", "params": self._key_id}))
        data = await self._next()
        stream = data.get("ev")
        msg = data.get("message")
        status = data.get("status")
        if stream == "status" and msg == "authenticated" and status == "auth_success":
    ...
    async def subscribe(self, channels):
        ...
        if len(channels) > 0:
            ...
            await self._ws.send(json.dumps({"action": "subscribe", "params": streams}))
